...
static DWORD I2C1MasterState = I2C_IDLE;
static BYTE I2C1MasterBuffer[BUFSIZE];
static DWORD I2C1WriteLength;
static DWORD WrIndex1 = 0;
static unsigned char lastI2c1Error = I2C_ERROR_NONE;
...
void fireFlyLedHandler(uint8_t gpsStatus) 	//called with 100Hz
{
  unsigned char r, g, b;
  ...
  if(sdk.ro.lowLevelError & FM_CALIBRATION_ERROR_GYROS)
  {
    errorFlags |= 0x02;
  }

  if(sdk.ro.lowLevelError & FM_CALIBRATION_ERROR_ACC)
  {
    errorFlags |= 0x04;
  }

  if((sdk.ro.lowLevelError & FM_COMPASS_FAILURE) && (ceCnt < 5))
  {
    errorFlags |= 0x08;
  }

  if((sdk.ro.lowLevelError & FM_MAG_INCLINATION_ERROR) && (mincCnt < 5)
      && ((sdk.ro.lowLevelError & FM_COMPASS_FAILURE) == 0))
  {
    errorFlags |= 0x10;
  }

  if((sdk.ro.lowLevelError & FM_MAG_FIELD_STRENGTH_ERROR) && (mfsCnt < 5)
      && ((sdk.ro.lowLevelError & FM_COMPASS_FAILURE) == 0))
  {
    errorFlags |= 0x20;
  }
  ...
  else if(((gpsStatus & 0xFF) == 3) && (HL_Status.battery_voltage_1 > 11000))
  {
    r = 0;
    g = 255;
    b = 0;
...
char I2C1SetRGBLed(unsigned char r, unsigned char g, unsigned char b)
{
